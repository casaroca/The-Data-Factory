...
echo "--- Setting up Common Crawl Query Collector ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/common_crawl_query_collector"
...
sudo bash -c "cat << EOF > /etc/systemd/system/common_crawl_query_collector.service
[Unit]
Description=Common Crawl Query Collector Service
...
sudo systemctl daemon-reload
sudo systemctl start common_crawl_query_collector
sudo systemctl enable common_crawl_query_collector

echo "--- Common Crawl Query Collector Setup Complete ---"
echo "To check the status, run: sudo systemctl status common_crawl_query_collector"

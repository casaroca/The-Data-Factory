...
echo "--- Setting up Salvage Extractor v2 (Factory-Wide) ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/processors/salvage_extractor"
...
sudo bash -c "cat << EOF > /etc/systemd/system/salvage_extractor.service
[Unit]
Description=Salvage Extractor Service v2 (Factory-Wide)
...
sudo systemctl daemon-reload
sudo systemctl start salvage_extractor
sudo systemctl enable salvage_extractor

echo "--- Salvage Extractor v2 Setup Complete ---"
echo "To check the status, run: sudo systemctl status salvage_extractor"

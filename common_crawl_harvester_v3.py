...
def clean_html(html_content_bytes):
    try:
        soup = BeautifulSoup(html_content_bytes, 'lxml-xml')
    except Exception:
        ...
        with gzip.open(temp_path, 'rb') as gz_file:
            for record in ArchiveIterator(gz_file):
                ...
                if (record.rec_type == 'response' and record.http_headers is not None and record.http_headers.get_statuscode() == '200'):
                    try:
                        if 'text/html' not in record.http_headers.get_header('content-type', '').lower() and 'application/xml' not in record.http_headers.get_header('content-type', '').lower(): continue
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for future in as_completed(future_to_archive):
                ...
                try:
                    ...
                    logging.error(f"Archive {future_to_archive[future]} generated an exception: {e}")

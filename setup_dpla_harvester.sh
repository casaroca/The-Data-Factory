...
echo "--- Setting up DPLA Harvester ---"

# --- 1. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/dpla_harvester"
...
cat << 'EOF' > $PROJECT_DIR/dpla_harvester.py
import os
import time
import logging
import requests
...
sudo bash -c "cat << EOF > /etc/systemd/system/dpla_harvester.service
[Unit]
Description=DPLA Harvester Service
...
sudo systemctl enable dpla_harvester

echo "--- DPLA Harvester Setup Complete ---"
echo "To check the status, run: sudo systemctl status dpla_harvester"

...
echo "--- Setting up FDLP Harvester v1 (Final Working Version) ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/fdlp_harvester"
...
cat << 'EOF' > $PROJECT_DIR/fdlp_harvester.py
import os
import time
import logging
import subprocess
...
IA_EXECUTABLE = "/factory/workers/collectors/fdlp_harvester/venv/bin/ia"
CYCLE_SLEEP_TIME = 6 * 60 # Reduced from 10 minutes
...
# --- 7. Configure 'ia' tool (if needed) ---
if [ ! -f "/home/$USER/.config/internetarchive/ia.ini" ]; then
    echo ""
...
sudo bash -c "cat << EOF > /etc/systemd/system/fdlp_harvester.service
[Unit]
Description=FDLP Harvester Service
...
sudo systemctl daemon-reload
sudo systemctl start fdlp_harvester
sudo systemctl enable fdlp_harvester

echo "--- FDLP Harvester Setup Complete ---"
echo "To check the status, run: sudo systemctl status fdlp_harvester"

...
echo "--- Setting up arXiv Harvester v3 ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/arxiv_harvester_v3"
...
sudo bash -c "cat << EOF > /etc/systemd/system/arxiv_harvester_v3.service
[Unit]
Description=arXiv Harvester Service v3
...
sudo systemctl daemon-reload
sudo systemctl start arxiv_harvester_v3
sudo systemctl enable arxiv_harvester_v3

echo "--- arXiv Harvester v3 Setup Complete ---"
echo "To check the status, run: sudo systemctl status arxiv_harvester_v3"

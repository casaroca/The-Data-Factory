...
echo "--- Setting up Archive.org Crawler v2 ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/archive_org_crawler_v2"
...
def get_item_links_from_page(page_url):
    ...
    for page_num in range(PAGES_TO_CRAWL):
        try:
            ...
            response = requests.get(current_url, headers={'User-Agent': 'ArchiveCrawler/2.0'})
...
def process_item_page(item_url):
    ...
    try:
        ...
        response = requests.get(item_url, headers={'User-Agent': 'ArchiveCrawler/2.0'}, timeout=60)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'lxml')
...
sudo bash -c "cat << EOF > /etc/systemd/system/archive_org_crawler_v2.service
[Unit]
Description=Archive.org Crawler Service v2
...
sudo systemctl daemon-reload
sudo systemctl start archive_org_crawler_v2
sudo systemctl enable archive_org_crawler_v2

echo "--- Archive.org Crawler v2 Setup Complete ---"
echo "To check the status, run: sudo systemctl status archive_org_crawler_v2"

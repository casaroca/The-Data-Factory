...
echo "--- Setting up Archive.org Query Collector ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/archive_org_query_collector"
...
sudo bash -c "cat << EOF > /etc/systemd/system/archive_org_query_collector.service
[Unit]
Description=Archive.org Query Collector Service
...
sudo systemctl daemon-reload
sudo systemctl start archive_org_query_collector
sudo systemctl enable archive_org_query_collector

echo "--- Archive.org Query Collector Setup Complete ---"
echo "To check the status, run: sudo systemctl status archive_org_query_collector"

...
echo "--- Setting up Public Dataset Harvester v2 ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/public_dataset_harvester_v2"
...
cat << 'EOF' > $PROJECT_DIR/public_dataset_harvester_v2.py
import os
import time
import logging
import sqlite3
...
sudo bash -c "cat << EOF > /etc/systemd/system/public_dataset_harvester_v2.service
[Unit]
Description=Public Dataset Harvester Service v2
...
sudo systemctl daemon-reload
sudo systemctl start public_dataset_harvester_v2
sudo systemctl enable public_dataset_harvester_v2

echo "--- Public Dataset Harvester v2 Setup Complete ---"
echo "To check the status, run: sudo systemctl status public_dataset_harvester_v2"

...
echo "--- Setting up Factory-Wide Image Processor ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/processors/image_processor"
...
cat << 'EOF' > $PROJECT_DIR/image_processor.py
import os
import time
import logging
import subprocess
...
sudo bash -c "cat << EOF > /etc/systemd/system/image_processor.service
[Unit]
Description=Factory-Wide Image Processor Service
...
# --- 7. Start the Service ---
echo "[+] Setting permissions and starting Image Processor service..."
sudo chown -R $USER:$USER $PROJECT_DIR $OUTPUT_DIR $(dirname $DB_PATH)
sudo systemctl daemon-reload
sudo systemctl start image_processor
sudo systemctl enable image_processor

echo "--- Image Processor Setup Complete ---"
echo "To check the status, run: sudo systemctl status image_processor"

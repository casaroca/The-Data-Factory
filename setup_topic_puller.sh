...
echo "--- Upgrading Topic Puller with Direct Scan Logic ---"

# --- 1. Define Paths ---
PROJECT_DIR="/factory/workers/extractors/topic_puller"
LOG_DIR="/factory/logs"
DB_PATH="/factory/db/topic_puller_log.db" # Using a dedicated DB now
LIBRARY_DIR="/factory/library/library" # The directory to scan
RAW_DUMP_DIR="/factory/data/inbox"
USER="tdf"
FACTORY_HOME="/factory"

# --- 2. Stop the service for upgrade ---
echo "[+] Stopping the topic_puller service..."
sudo systemctl stop topic_puller || true

# --- 3. Create Application Files ---
echo "[+] Creating upgraded topic_puller.py application file..."
...
MAX_WORKERS = 10
DB_TIMEOUT = 30.0
CYCLE_TIME_SECONDS = 60 # Changed from 5 minutes to 60 seconds
...
def extract_text_from_epub(epub_path):
    ...
    try:
        ...
        for item in book.get_items_of_type(ebooklib.ITEM_DOCUMENT):
            soup = BeautifulSoup(item.get_body_content(), 'lxml', from_encoding='utf-8')
...
# --- 4. Start the Service ---
echo "[+] Starting Topic Puller service with new logic..."
sudo systemctl restart topic_puller

echo "--- Topic Puller Upgrade Complete ---"

...
echo "--- Setting up Common Crawl Bulk Collector ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/common_crawl_bulk_collector"
...
sudo bash -c "cat << EOF > /etc/systemd/system/common_crawl_bulk_collector.service
[Unit]
Description=Common Crawl Bulk Collector Service
...
sudo systemctl daemon-reload
sudo systemctl start common_crawl_bulk_collector
sudo systemctl enable common_crawl_bulk_collector

echo "--- Common Crawl Bulk Collector Setup Complete ---"
echo "To check the status, run: sudo systemctl status common_crawl_bulk_collector"

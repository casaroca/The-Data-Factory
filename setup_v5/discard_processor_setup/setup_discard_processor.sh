...
echo "--- Setting up Discard Processor ---"

# --- 1. Define Paths ---
PROJECT_DIR="/factory/workers/processors/discard_processor"
...
logging.basicConfig(filename=os.path.join(LOG_DIR, 'discard_processor.log'), level=logging.INFO, format='%(asctime)s - %(message)s')
...
sudo bash -c "cat << EOF > /etc/systemd/system/discard_processor.service
[Unit]
Description=Discard Processor Service
...
# --- 7. Start the Service ---
echo "[+] Setting permissions and starting Discard Processor service..."
sudo chown -R $USER:$USER $PROJECT_DIR $OUTPUT_DIR
sudo systemctl daemon-reload
sudo systemctl start discard_processor
sudo systemctl enable discard_processor

echo "--- Discard Processor Setup Complete ---"
echo "To check the status, run: sudo systemctl status discard_processor"

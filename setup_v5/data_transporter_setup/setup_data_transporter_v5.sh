...
echo "--- Setting up Data Transporter v1 v5 ---"

# --- 1. Define Paths ---
PROJECT_DIR="/factory/workers/organizers/data_transporter_v5"
...
sudo bash -c "cat << EOF > /etc/systemd/system/data_transporter_v5.service
[Unit]
Description=Data Transporter Service (OVH to Local) v5
...
sudo systemctl start data_transporter_v5
sudo systemctl enable data_transporter_v5

echo "--- Data Transporter Setup Complete v5 ---"
echo "To check the status, run: sudo systemctl status data_transporter_v5"

...
echo "--- Setting up Archive.org Harvester v2 (Fixed) v5 ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/archive_org_harvester_v2_v5"
...
IA_EXECUTABLE = "/factory/workers/collectors/archive_org_harvester_v2_v5/venv/bin/ia"
...
sudo bash -c "cat << EOF > /etc/systemd/system/archive_org_harvester_v2_v5.service
[Unit]
Description=Archive.org Harvester Service v2 v5
...
sudo chown -R $USER:$USER $PROJECT_DIR $(dirname $DB_PATH)
sudo systemctl daemon-reload
sudo systemctl start archive_org_harvester_v2_v5
sudo systemctl enable archive_org_harvester_v2_v5

echo "--- Archive.org Harvester v2 Setup Complete v5 ---"
echo "To check the status, run: sudo systemctl status archive_org_harvester_v2_v5"

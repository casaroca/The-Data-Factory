...
echo "--- Setting up Targeted Ebook Collector v3 (Corrected Crawler) v5 ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/targeted_collector_v3_v5"
...
sudo bash -c "cat << EOF > /etc/systemd/system/targeted_collector_v3_v5.service
[Unit]
Description=Targeted Ebook Collector Service v3 v5
...
sudo systemctl daemon-reload
sudo systemctl start targeted_collector_v3_v5
sudo systemctl enable targeted_collector_v3_v5

echo "--- Targeted Ebook Collector Setup Complete v5 ---"
echo "To check the status, run: sudo systemctl status targeted_collector_v3_v5"

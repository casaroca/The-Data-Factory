...
echo "--- Setting up Targeted Ebook Collector v6 (Robust Link Finder) v5 ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/targeted_collector_v6_v5"
...
sudo bash -c "cat << EOF > /etc/systemd/system/targeted_collector_v6_v5.service
[Unit]
Description=Targeted Ebook Collector Service v6 v5
...
sudo systemctl daemon-reload
sudo systemctl start targeted_collector_v6_v5
sudo systemctl enable targeted_collector_v6_v5

echo "--- Targeted Ebook Collector v6 Setup Complete v5 ---"
echo "To check the status, run: sudo systemctl status targeted_collector_v6_v5"

...
echo "--- Setting up Outlier Detector v1 v5 ---"

# --- 1. Define Paths ---
PROJECT_DIR="/factory/workers/processors/outlier_detector_v1_v5"
...
mv /home/tdf/setup_v5/outlier_detector_v1_setup/outlier_detector_v1.py $PROJECT_DIR/outlier_detector_v1.py
...
sudo bash -c "cat << EOF > /etc/systemd/system/outlier_detector_v1_v5.service
[Unit]
Description=Semantic Outlier Detector Service v1 v5
...
sudo systemctl start outlier_detector_v1_v5
sudo systemctl enable outlier_detector_v1_v5

echo "--- Outlier Detector v1 Setup Complete v5 ---"
echo "To check the status, run: sudo systemctl status outlier_detector_v1_v5"
echo "To watch the logs, run: tail -f /factory/logs/outlier_detector.log"

...
echo "--- Setting up Archive.org Crawler v3 (API-Powered) ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/archive_org_crawler_v3"
...
sudo bash -c "cat << EOF > /etc/systemd/system/archive_org_crawler_v3.service
[Unit]
Description=Archive.org Crawler Service v3
...
sudo systemctl daemon-reload
sudo systemctl start archive_org_crawler_v3
sudo systemctl enable archive_org_crawler_v3

echo "--- Archive.org Crawler v3 Setup Complete ---"
echo "To check the status, run: sudo systemctl status archive_org_crawler_v3"

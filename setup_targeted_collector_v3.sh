...
echo "--- Setting up Targeted Ebook Collector v3 (Corrected Crawler) ---"

# --- 1. Stop and remove all old versions to prevent conflicts ---
echo "[+] Stopping and removing all old targeted_collector services..."
sudo systemctl stop targeted_collector targeted_collector_v2 || true
sudo systemctl disable targeted_collector targeted_collector_v2 || true
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/targeted_collector_v3"
...
echo "[+] Creating targeted_collector_v3.py application file..."
cat << 'EOF' > $PROJECT_DIR/targeted_collector_v3.py
import os
import time
import logging
import requests
...
logging.basicConfig(filename=os.path.join(LOG_DIR, 'targeted_collector_v3.log'), level=logging.INFO, format='%(asctime)s - %(message)s')
...
def download_book(ebook_page_url):
    ...
    try:
        ...
        response = requests.get(ebook_page_url, headers={'User-Agent': 'TargetedCollector/3.0'})
        ...
        book_content_response = requests.get(download_url, headers={'User-Agent': 'TargetedCollector/3.0'})
...
def crawl_collection(start_url):
    ...
    for _ in range(PAGES_TO_CRAWL):
        try:
            ...
            response = requests.get(current_url, headers={'User-Agent': 'TargetedCollector/3.0'})
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'lxml')
            
            # Corrected selector for item links on collection pages
            page_links = {urljoin(current_url, a['href']) for a in soup.select('div.results-item a.title-link')}
            if not page_links:
                 page_links = {urljoin(current_url, a['href']) for a in soup.select('div.item-tile > a')}
...
def main():
    while True:
        logging.info("--- Starting new Targeted Collector v3 cycle ---")
...
sudo bash -c "cat << EOF > /etc/systemd/system/targeted_collector_v3.service
[Unit]
Description=Targeted Ebook Collector Service v3
...
ExecStart=$PROJECT_DIR/venv/bin/python3 targeted_collector_v3.py
Restart=always
RestartSec=10
...
# --- 7. Start the Service ---
echo "[+] Starting Targeted Ebook Collector v3 service..."
sudo chown -R $USER:$USER /factory /library
sudo systemctl daemon-reload
sudo systemctl start targeted_collector_v3
sudo systemctl enable targeted_collector_v3

echo "--- Targeted Ebook Collector v3 Setup Complete ---"
echo "To check the status, run: sudo systemctl status targeted_collector_v3"
echo "To watch the logs, run: tail -f /factory/logs/targeted_collector_v3.log"

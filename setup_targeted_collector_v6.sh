...
echo "--- Setting up Targeted Ebook Collector v6 (Robust Link Finder) ---"

# --- 1. Stop and remove all old versions to prevent conflicts ---
echo "[+] Stopping and removing all old targeted_collector services..."
sudo systemctl stop targeted_collector targeted_collector_v2 targeted_collector_v3 targeted_collector_v4 targeted_collector_v5 || true
sudo systemctl disable targeted_collector targeted_collector_v2 targeted_collector_v3 targeted_collector_v4 targeted_collector_v5 || true
sudo rm -f /etc/systemd/system/targeted_collector*.service
sudo rm -rf /factory/workers/collectors/targeted_collector*
sudo systemctl daemon-reload

# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/collectors/targeted_collector_v6"
...
echo "[+] Creating targeted_collector_v6.py application file..."
cat << 'EOF' > $PROJECT_DIR/targeted_collector_v6.py
import os
import time
import logging
import requests
...
logging.basicConfig(filename=os.path.join(LOG_DIR, 'targeted_collector_v6.log'), level=logging.INFO, format='%(asctime)s - %(message)s')
logging.getLogger('').addHandler(logging.StreamHandler())

def download_book(ebook_page_url):
    """Visits a final item page, finds the best available format, and downloads it."""
    try:
        logging.info(f"Processing book page: {ebook_page_url}")
        response = requests.get(ebook_page_url, headers={'User-Agent': 'TargetedCollector/6.0'})
        ...
        book_content_response = requests.get(download_url, headers={'User-Agent': 'TargetedCollector/6.0'})
...
def crawl_collection(start_url):
    """Crawls a collection by finding all item links and following the 'Next' page."""
    all_links = set()
    ...
    for _ in range(PAGES_TO_CRAWL):
        try:
            ...
            response = requests.get(current_url, headers={'User-Agent': 'TargetedCollector/6.0'})
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'lxml')
            
            # THE FIX: A more robust method to find all links that lead to an item details page.
            page_links = {urljoin(current_url, a['href']) for a in soup.find_all('a', href=True) if '/details/' in a['href']}
...
def main():
    while True:
        logging.info("--- Starting new Targeted Collector v6 cycle ---")
...
sudo bash -c "cat << EOF > /etc/systemd/system/targeted_collector_v6.service
[Unit]
Description=Targeted Ebook Collector Service v6
...
ExecStart=$PROJECT_DIR/venv/bin/python3 targeted_collector_v6.py
Restart=always
RestartSec=10
...
# --- 7. Start the Service ---
echo "[+] Starting Targeted Ebook Collector v6 service..."
sudo chown -R $USER:$USER /factory /library
sudo systemctl daemon-reload
sudo systemctl start targeted_collector_v6
sudo systemctl enable targeted_collector_v6

echo "--- Targeted Ebook Collector v6 Setup Complete ---"
echo "To check the status, run: sudo systemctl status targeted_collector_v6"
echo "To watch the logs, run: tail -f /factory/logs/targeted_collector_v6.log"

...
echo "--- Setting up Media Processor ---"
...
# --- 2. Define Paths ---
PROJECT_DIR="/factory/workers/processors/media_processor"
...
sudo bash -c "cat << EOF > /etc/systemd/system/media_processor.service
[Unit]
...
sudo systemctl start media_processor
sudo systemctl enable media_processor

echo "--- Media Processor Setup Complete ---"
echo "To check the status, run: sudo systemctl status media_processor"

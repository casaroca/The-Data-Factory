...
echo "--- Setting up Topic Puller v2 (with PDF Salvage) ---"
...
DISCARD_BIN = "/factory/library/discarded"
SALVAGED_OUTPUT_DIR = "/factory/data/raw/salvaged_from_discard"
UNSALVAGEABLE_DIR = "/library/discarded/unsalvageable_pdfs"
MAX_WORKERS = 10
DB_TIMEOUT = 30.0
...
[Unit]
Description=Ebook Topic Puller Service v2
...
sudo chown -R $USER:$USER $PROJECT_DIR $SALVAGED_OUTPUT_DIR $UNSALVAGEABLE_DIR
sudo systemctl daemon-reload
sudo systemctl start topic_puller
sudo systemctl enable topic_puller

echo "--- Topic Puller v2 Setup Complete ---"
